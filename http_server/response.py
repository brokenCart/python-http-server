...
class Response:
    ...
    def build(self):
        delimeter = "\r\n"
        response_string = f"{self.version} {self.status_code} {HTTP_STATUS_CODES[self.status_code]}" + delimeter
        response_string += delimeter.join([f"{key}: {self.headers[key]}" for key in self.headers]) + delimeter
